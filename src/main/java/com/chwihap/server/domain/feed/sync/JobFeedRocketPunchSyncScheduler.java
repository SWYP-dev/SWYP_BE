...
/**
 * 로켓펀치 채용공고 수집 배치 트리거. 다른 채용정보 수집과 동일하게 하루 2회(08시·20시) 수집한다.
 * <p>
...
public class JobFeedRocketPunchSyncScheduler {

    private static final String LOCK_KEY = "lock:job-feed-rocketpunch-sync";
    private static final long LEASE_TIME_MINUTES = 10;
    private static final String JOB_TAG = "rocketpunch";
    ...
    public void sync() {
        ...
        try {
            ...
            if (!acquired) {
                log.info("다른 인스턴스가 이미 로켓펀치 공고 수집 중이라 이번 실행은 스킵합니다.");
                return;
            }
            log.info("로켓펀치 공고 수집 스케줄러 시작");
            ...
            log.warn("로켓펀치 공고 수집 락 획득 대기 중 인터럽트가 발생했습니다.", e);
        } finally {
